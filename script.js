...
function loadContent(type) {
    ...
    container.innerHTML = ids.map(id => {
        ...
        return q ? generateQuestionHTML(q, type) : '';
    }).join('');
...
function loadKnowledgePoints() {
    ...
    const combinedIds = [...new Set([...errorIds, ...favoriteIds, ...notedIds])].sort((a, b) => parseInt(a) - parseInt(b));
...
// --- HTML Generators ---
function generateQuestionHTML(q, pageType) {
    ...
    const options_html = Object.entries(opts).map(([k, v]) => `<label><input type="${q_type === '多选' ? 'checkbox' : 'radio'}" name="q-${q.id}" value="${k}"><span>${k}</span> ${v}</label>`).join('');
    
    let actionButtons = `<button class="icon-btn favorite-btn" onclick="toggleFavorite('${q.id}')" title="收藏"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"></polygon></svg></button>`;
    if (pageType === 'errors' || pageType === 'favorites') {
        actionButtons += `<button class="icon-btn remove-btn" onclick="removeStoredItem('${pageType}', '${q.id}')" title="移除"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg></button>`;
    }

    return `
    <div class="question-block" id="q-${q.id}" data-answer="${normalized_answer}">
        <div class="question-header">
            <div class="question-title"><span class="question-id">${q.id}</span><p>【${q_type}】${q.question}</p></div>
            <div class="question-actions">${actionButtons}</div>
        </div>
        <div class="options">${options_html}</div>
        <div class="feedback" id="feedback-q-${q.id}"></div>
        <div class="note-section">
            <details><summary><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"></path><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"></path></svg><span>我的笔记</span></summary>
                <textarea id="note-q-${q.id}" placeholder="在此输入您的笔记..."></textarea>
                <button class="save-note-btn" onclick="saveNote('${q.id}')">保存笔记</button>
            </details>
        </div>
        <button class="check-btn" onclick="checkAnswer('${q.id}')">确认</button>
    </div>`;
}
...
function setupActionButtons(idsToExport) {
    document.getElementById('export-md-btn').addEventListener('click', () => {
        let markdownContent = "# 核心知识库 (错题、收藏与笔记)\n\n";
        idsToExport.forEach(id => {
            const q = ALL_QUESTIONS.find(q => q.id == id);
            if (!q) return;
            const correct_keys = Array.isArray(q.answer) ? q.answer.sort() : [String(q.answer)];
            markdownContent += `### ${q.id}. ${q.question}\n\n`;
            let opts = q.options;
            if (Array.isArray(opts)) opts = Object.fromEntries(opts.map((v, i) => [String.fromCharCode(65 + i), v]));
            for (const [key, value] of Object.entries(opts)) {
                markdownContent += `* ${correct_keys.includes(key) ? '**' : ''}${key}. ${value}${correct_keys.includes(key) ? '**' : ''}\n`;
            }
            markdownContent += `\n**答案：${correct_keys.join('')}**\n`;
            const note = getStore('maogaiNotes')[q.id];
        ...
        a.download = 'My_Knowledge_Base.md';
        a.click();
